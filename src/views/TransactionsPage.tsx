    ...
    function validateBeforeSubmit() {
        ...
        if (headerForm.hasDetails) {
            ...
            if (totalDetailsMinor !== headerMinor) {
                return {
                    ok: false,
                    message: `La somma dei dettagli (${formatMinorHuman(totalDetailsMinor, headerForm.amount_decimal)}) non coincide con l'importo della testata (${headerForm.amountStr}).`
                };
            }
        } else {
            if (!headerForm.amountStr || headerForm.amountStr.trim() === '') return {
                ok: false,
                message: 'Inserisci l\'importo della transazione.'
            };
        }
        ...
        if (headerForm.type === 'transfer') {
            if (!headerForm.from_account_id || !headerForm.to_account_id) return {
                ok: false,
                message: 'Per trasferimenti serve conto di origine e destinazione.'
            };
            if (headerForm.from_account_id === headerForm.to_account_id) return {
                ok: false,
                message: 'Origine e destinazione non possono essere lo stesso conto.'
            };
        }
                ...
                <div className="col-span-8 bg-base-200 rounded-lg p-4 overflow-auto max-h-[70vh] min-w-0">
                    ...
                    <div className="space-y-3">
                                ...
                                <div className="card-body p-3 flex items-center gap-3">
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center justify-between">
                                            <div
                                                className="font-medium truncate">{h.description ?? '(no description)'}</div>
                                            <div
                                                className="text-sm font-mono">{formatMinorToDecimal(h.amount as number, h.amount_decimal)} {h.currency}</div>
                                        </div>
                                        <div className="text-xs opacity-60 mt-1">Type: <span
                                            className="font-medium">{h.type}</span> • Cat: {h.category_id ?? '—'}</div>
                                        <div className="text-xs opacity-50">From {h.from_account_id ?? '—'} →
                                            To {h.to_account_id ?? '—'} • {new Date(h.transaction_date).toLocaleString()}</div>
                                    </div>

                                    <div className="flex flex-col items-end gap-2">
                                        <div className="flex gap-2">
                                            <button onClick={() => openEdit(h.id)} className="btn btn-ghost btn-sm"
                                                    title="Edit">
                                                <Pencil size={16}/>
                                            </button>
                                            <button onClick={() => handleDeleteHeader(h.id)}
                                                    className="btn btn-ghost btn-sm text-error" title="Delete">
                                                <Trash size={16}/>
                                            </button>
                                        </div>
                                    </div>
                        ...
                        {visibleHeaders.length === 0 &&
                            <div className="text-sm opacity-60 p-4">No transactions found.</div>}
                    </div>
                </div>
                                ...
                                <form onSubmit={handleSubmit} className="flex flex-col gap-3">
                                    ...
                                    <div className="form-control">
                                        ...
                                        <CategorySelector<AccountCategory>
                                            ...
                                            onChange={(id) => setHeaderForm(prev => ({
                                                ...prev,
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                        <div className="form-control">
                                            <label className="label"><span
                                                className="label-text">From account</span></label>
                                            <AccountSelector
                                                accounts={accounts}
                                        ...
                                        <div className="form-control">
                                            <label className="label"><span
                                                className="label-text">To account</span></label>
                                            <AccountSelector
                                                accounts={accounts}
                                    ...
                                    <div className="grid grid-cols-3 gap-2">
                                        <div className="form-control">
                                            ...
                                            <select className="select select-sm select-bordered w-full"
                                                    value={headerForm.type} onChange={e => setHeaderForm(prev => ({
                                                ...prev,
                                                type: e.target.value
                                            }))}>
                                        ...
                                        <div className="form-control">
                                            ...
                                            <select className="select select-sm select-bordered w-full"
                                                    value={headerForm.method} onChange={e => setHeaderForm(prev => ({
                                                ...prev,
                                        ...
                                        <div className="form-control">
                                            ...
                                            <select className="select select-sm select-bordered w-full"
                                                    value={headerForm.status} onChange={e => setHeaderForm(prev => ({
                                                ...prev,
                                    ...
                                    <div className="grid grid-cols-1 gap-2">
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            <div className="form-control">
                                                <label className="label"><span
                                                    className="label-text">Currency</span></label>
                                            ...
                                            <div className="form-control">
                                                <label className="label"><span
                                                    className="label-text">Transaction date</span></label>
                                                <input type="datetime-local"
                                                       value={toInputLocal(headerForm.transactionDateISO)}
                                                       onChange={e => setHeaderForm(prev => ({
                                                           ...prev,
                                                           transactionDateISO: fromInputLocal(e.target.value)
                                                       }))} className="input input-sm input-bordered"/>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div className="border rounded p-2 bg-base-200">
                                            <div className="flex items-center justify-between mb-2">
                                                ...
                                                <button type="button" className="btn btn-sm btn-ghost"
                                                        onClick={addDetailRow}><Plus size={14}/></button>
                                            </div>
                                                ...
                                                {detailRows.map(row => (
                                                    <div key={row.id}
                                                        ...
                                                        <div className="col-span-2">
                                                            <label className="label"><span
                                                                className="label-text text-xs">Description</span></label>
                                                            <input className="input input-sm input-bordered w-full"
                                                                   value={row.description || ''}
                                                                   onChange={e => updateDetailRow(row.id, {description: e.target.value})}/>
                                                        </div>
                                                        <div>
                                                            <label className="label"><span
                                                                className="label-text text-xs">Amount</span></label>
                                                            <div className="flex gap-2">
                                                                <input className="input input-sm input-bordered w-full"
                                                                       value={row.amountStr}
                                                                       onChange={e => updateDetailRowAmount(row.id, e.target.value)}/>
                                                                <button type="button" className="btn btn-ghost btn-sm"
                                                                        onClick={() => removeDetailRow(row.id)}
                                                                        title="Remove"><Trash size={14}/></button>
                                                            </div>
                                                        </div>
                                                    </div>
                                                ))}
                                            ...
                                            <div className="mt-3 text-sm">
                                                <strong>Sum
                                                    details:</strong> {formatMinorToDecimal(totalDetailsMinor, headerForm.amount_decimal)}
                                                <span
                                                    className="ml-3 text-xs opacity-60">Header: {headerForm.amountStr}</span>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    <div className="form-control">
                                        ...
                                        <input className="input input-sm input-bordered" value={headerForm.description}
                                               onChange={e => setHeaderForm(prev => ({
                                                   ...prev,
                                    ...
                                    <div className="form-control">
                                        <label className="label"><span
                                            className="label-text">Notes / Tags</span></label>
                                        <input className="input input-sm input-bordered" value={headerForm.notes}
                                               onChange={e => setHeaderForm(prev => ({
                                                   ...prev,
                                                   notes: e.target.value
                                               }))}/>
                                        <input className="input input-sm input-bordered mt-2" value={headerForm.tags}
                                               onChange={e => setHeaderForm(prev => ({...prev, tags: e.target.value}))}
                                    ...
                                    <div className="grid grid-cols-3 gap-2 items-end">
                                        <div className="form-control">
                                            ...
                                            <input type="color" className="w-12 h-10 p-1 rounded"
                                                   value={headerForm.color} onChange={e => setHeaderForm(prev => ({
                                                ...prev,
                                        ...
                                        <div className="form-control col-span-2">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <input className="input input-sm input-bordered flex-1" value={formIcon}
                                                       onChange={e => {
                                                           setFormIcon(e.target.value);
                                                           setHeaderForm(prev => ({...prev, icon: e.target.value}));
                                                       }}/>
                                                <button type="button" className="btn btn-ghost btn-sm"
                                                        onClick={() => setPickerOpen(true)}>Scegli icona
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex justify-end gap-2 mt-2">
                                        <button type="button" onClick={() => {
                                            ...
                                            setInserting(false);
                                            setDetailRows([]);
                                        }} className="btn btn-ghost btn-sm">Cancel
                                        </button>
                ...
                <div className="modal modal-open">
                    <div className="modal-box max-w-3xl">
                        <div className="flex items-center justify-between mb-3">
                            ...
                            <button className="btn btn-ghost btn-sm" onClick={() => setPickerOpen(false)}>Chiudi
                            </button>
                        </div>
                            ...
                            {ICONS
                                ...
                                .map(ic => (
                                    <button
                                        ...
                                        onClick={() => {
                                            setFormIcon(ic);
                                            setHeaderForm(prev => ({...prev, icon: ic}));
                                            setPickerOpen(false);
                                        }}
                                        onKeyDown={(e) => {
