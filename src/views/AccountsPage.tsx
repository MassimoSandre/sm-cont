                    ...
                    {acc.children && acc.children.length > 0 ? (
                        ...
                        >
                            <svg xmlns="http://www.w3.org/2000/svg"
                                 className={`h-4 w-4 transform ${expanded ? 'rotate-90' : ''}`} viewBox="0 0 24 24"
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full border flex items-center justify-center text-sm"
                             style={{backgroundColor: acc.color ?? '#fff'}} aria-hidden/>
                        <div className="min-w-0">
                            <div className="text-sm font-medium truncate">{acc.name}</div>
                            <div className="text-xs opacity-60 truncate">{acc.description ?? '—'}</div>
                        </div>
                    </div>
                    <div className="mt-1 text-xs text-muted">Type: <span
                        className="font-medium">{acc.type || '—'}</span> • Cat: <span
                        className="font-medium">{acc.category_id ?? '—'}</span></div>
                </div>

                {/* right: balance */}
                <div className="flex-none text-right mr-3">
                    <div
                        className="font-mono text-sm">{formatBalanceFromCents(acc.balance, acc.balance_decimal ?? 2, acc.currency)}</div>
            ...
            <div className="grid grid-cols-12 gap-6">
                ...
                <div className="col-span-8 bg-base-200 rounded-lg p-4 overflow-auto max-h-[70vh] min-w-0">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <div
                            className="badge badge-outline">{Object.keys(totals).map(c => `${c} ${totals[c].toFixed(2)}`).join(' • ')}</div>
                    </div>
                    ...
                    <div>
                        ...
                        {visibleAccounts.length === 0 &&
                            <div className="text-sm opacity-60 p-4">No accounts found.</div>}
                    </div>
                </div>
                                ...
                                <form onSubmit={handleSubmit} className="flex flex-col gap-3">
                                    ...
                                    <div className="form-control">
                                        <label className="label"><span
                                            className="label-text">Parent account</span></label>
                                        <AccountSelector
                                            accounts={accounts}
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        <div className="form-control">
                                            ...
                                            />
                                            <label className="label"><span
                                                className="label-text-alt text-xs opacity-60">Enter euros (comma or dot allowed). We store as cents.</span></label>
                                        </div>
                                    ...
                                    <div className="grid grid-cols-3 gap-2 items-end">
                                        <div className="form-control col-span-2">
                                            ...
                                            <input className="input input-bordered input-sm" value={form.type}
                                                   onChange={e => setForm({...form, type: e.target.value})}/>
                                        </div>

                                        <div className="form-control">
                                            <label className="label"><span className="label-text">Color</span></label>
                                            <input type="color" className="w-12 h-10 p-1 rounded" value={form.color}
                                    ...
                                    <div className="form-control">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <input className="input input-bordered input-sm flex-1" value={formIcon}
                                                   onChange={(e) => {
