...
type CategoryNode = NodeModel<TransactionCategory>;

const buildTreeNodes = (cats: TransactionCategory[] | undefined): CategoryNode[] => {
    if (!cats) return [];
    return cats.map(c => ({
        id: c.id,
...
const CustomNode: React.FC<{
    ...
    const cat = node.data as TransactionCategory;
    ...
    return (
        ...
        >
            <div className="flex items-center gap-3">
                ...
                >
                    <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 transform ${isOpen ? 'rotate-90' : ''}`}
                ...
                <div className="w-10 h-10 rounded-full border flex items-center justify-center"
                     style={{backgroundColor: cat.color}} aria-hidden/>
...
export const TransactionsCategoryPage: React.FC = () => {
    ...
    } = useTransactionsCategoryViewModel();

    const [treeData, setTreeData] = useState<CategoryNode[]>(() => buildTreeNodes(transactionCategories));
    ...
    useEffect(() => {
        setFormIcon(selected?.data?.icon ?? 'fa-folder');
    }, [selected]);

    useEffect(() => {
        setTreeData(buildTreeNodes(transactionCategories));
    }, [transactionCategories]);
    ...
    const filteredTree = useMemo(() => {
        ...
        const nodeMatches = (n: CategoryNode) => {
            const cat = n.data as TransactionCategory;
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (editing) {
            ...
            try {
                await updateTransactionCategory(selected.id as number, {
    ...
    const confirmDelete = async () => {
        ...
        if (selected && (selected.id === deleteTarget.id || selected.parent === deleteTarget.id)) {
            setSelected(null);
            setEditing(false);
            setInserting(false);
        }
        try {
            await deleteTransactionCategory(deleteTarget.id as number);
            await fetchCategories();
        } catch (err) {
    ...
    return (
        ...
        <div className="p-6 w-full max-w-full min-w-0 flex-1">
                ...
                <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="join flex-shrink-0">
                        <button className="btn btn-sm join-item"
                                onClick={() => setTreeData(buildTreeNodes(transactionCategories))}>Reset
                        </button>
                        <button className="btn btn-primary btn-sm join-item" onClick={() => {
                            setSelected(null);
                            setInserting(true);
                            setEditing(false);
                        }}>New category
            ...
            <div className="grid grid-cols-12 gap-6 w-full max-w-full min-w-0">
                ...
                <div
                    className={`${(inserting || editing) ? 'col-span-8' : 'col-span-12'} bg-base-200 rounded-lg p-4 overflow-auto max-h-[70vh] w-full min-w-0`}>
                        ...
                        <DndProvider backend={HTML5Backend}>
                            <Tree
                                ...
                                dragPreviewRender={(monitorProps) => (
                                    <div
                                        className="p-2 bg-white rounded shadow-lg border">{monitorProps.item.text}</div>
                                )}
                                onDrop={() => {
                                }}
                            />
                    ...
                    <aside className="col-span-4">
                        <div className="card bg-base-100 shadow-sm">
                            <div className="card-body">
                                ...
                                <form onSubmit={handleSubmit} className="flex flex-col gap-3"
                                    ...
                                    <label className="text-xs opacity-60">Parent category</label>
                                    <CategorySelector
                                    ...
                                    <input name="name" defaultValue={selected?.data?.name ?? ''} placeholder="Nome"
                                           className="input input-bordered" required/>
                                    <input name="description" defaultValue={selected?.data?.description ?? ''}
                                           placeholder="Descrizione" className="input input-bordered"/>

                                    <div className="flex gap-2">
                                        <input name="type" defaultValue={selected?.data?.type ?? ''} placeholder="Tipo"
                                        ...
                                        <div className="flex items-center gap-2">
                                            <button type="button" className="btn btn-ghost"
                                                    onClick={() => setPickerOpen(true)} aria-haspopup="dialog"
                        ...
                        <i className={`fa ${formIcon} fa-fw`}/>
                                    ...
                                    <div className="flex justify-end gap-2">
                                        ...
                                        <button type="submit"
                                                className="btn btn-primary">{editing ? 'Save' : 'Insert'}</button>
                                    </div>
                                </form>
                                        ...
                                        <div className="modal-box max-w-3xl">
                                            <div className="flex items-center justify-between mb-3">
                                                ...
                                                <button className="btn btn-ghost btn-sm"
                                                        onClick={() => setPickerOpen(false)}>Chiudi
                                                </button>
                                            </div>
                                                ...
                                                {ICONS
                                                    ...
                                                    .map(ic => (
                                                        ...
                                                        >
                                                            ...
                                                            <div
                                                                className="text-xs mt-1 truncate">{ic.replace('fa-', '')}</div>
                                                        </button>
                                                    ))
                                                }
                                            </div>
                                ...
                                <div className="mt-4">
                                    ...
                                    <div
                                        className="text-xs opacity-60">Selected: {selected ? (selected.data as TransactionCategory).name : 'Nessuno'}</div>
                                </div>
                            </div>
                        </div>
                    </aside>)}
            ...
            {deleteTarget && (
                <div className="modal modal-open">
                    <div className="modal-box">
                        ...
                        <p className="py-4">Are you sure that you want to delete
                            category <strong>{deleteTarget?.data?.name}</strong>? This action cannot be undone.</p>
