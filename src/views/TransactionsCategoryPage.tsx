...
export const TransactionsCategoryPage: React.FC = () => {
  const { transactionCategories, addTransactionCategory, fetchCategories, updateTransactionCategory, deleteTransactionCategory, loading } = useTransactionsCategoryViewModel();
  ...
  <div className="flex items-center gap-3 min-w-0">
    ...
    <div className="join flex-shrink-0">
      <button className="btn btn-sm join-item" onClick={() => setTreeData(buildTreeNodes(transactionCategories))}>Reset</button>
            ...
            <div className="card-body">
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-3" aria-labelledby="category-form">
                ...
                <div className="flex justify-end gap-2">
                  <button type="button" className="btn btn-ghost" onClick={() => {setSelected(null); setEditing(false); setInserting(false);}}>Cancel</button>
                  <button type="submit" className="btn btn-primary">{editing ? 'Save' : 'Insert'}</button>
                </div>
              </form>
              ...
              <div className="mt-4">
                ...
                <div className="text-xs opacity-60">Selected: {selected ? (selected.data as TransactionCategory).name : 'Nessuno'}</div>
              </div>
            </div>
          </div>
        </aside>)}
