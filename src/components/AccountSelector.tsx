...
export default function AccountSelector({
  ...
  useEffect(() => {
    ...
    if (treeRef.current && toOpen.size) {
      ...
      Promise.resolve().then(() => {
        try { treeRef.current.open(ids); } catch (err) { /* ignore */ }
      });
    }
  }, [filter, treeData]);
  ...
  const SelectNode: React.FC<{ node: AccountNode; depth: number; isOpen: boolean; onToggle: () => void; }> = ({ node, depth, isOpen, onToggle }) => {
    ...
    const handleToggle = (e: React.MouseEvent) => {
      ...
      if (isOpen && treeRef.current) {
        ...
        if (desc.length > 0) {
          Promise.resolve().then(() => {
            try { treeRef.current.close(desc); } catch (err) { /* ignore */ }
          });
        }
      }
    };
    ...
    return (
      ...
      >
        <div className="w-8 h-8 rounded-full border flex items-center justify-center" style={{ backgroundColor: acc.color ?? '#ffffff' }} aria-hidden />
        ...
        <div className="ml-auto">
          <button className="btn btn-ghost btn-xs p-1" onClick={handleToggle} aria-label={isOpen ? 'Collapse' : 'Expand'} disabled={disabled}>
            <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 transform ${isOpen ? 'rotate-90' : ''}`} viewBox="0 0 24 24" fill="none" stroke="currentColor"><path strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
          </button>
        </div>
      </div>
    );
      ...
      <div className={`flex items-center gap-2 ${className ?? ''}`}>
        ...
        >
          <div className="flex items-center gap-2 min-w-0 truncate">
            {selectedNode ? (
              <>
                <div className="w-6 h-6 rounded-full border flex items-center justify-center" style={{ backgroundColor: (selectedNode.data as Account).color ?? '#fff' }} />
          ...
          <div className="ml-2 opacity-70">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
          </div>
        </button>

        {allowRootChoice && (
          <button type="button" className="btn btn-ghost" onClick={clearSelection} title="Rimuovi selezione">✕</button>
        )}
      </div>
          ...
          <div className="modal-box max-w-4xl w-full">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex gap-2">
                ...
                <button className="btn btn-primary btn-sm" onClick={() => confirmSelection(selectedId ?? null)} disabled={selectionIsProhibited}>Seleziona</button>
              </div>
            </div>
              ...
              <div className="w-2/5">
                <div className="card bg-base-100 shadow-sm p-3">
                    ...
                    {selectedNode ? (
                      <>
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full border flex items-center justify-center" style={{ backgroundColor: (selectedNode.data as Account).color ?? '#fff' }} />
                          <div>
                            <div className="font-semibold">{(selectedNode.data as Account).name}</div>
                            <div className="text-xs opacity-60">{(selectedNode.data as Account).description}</div>
                          </div>
                        </div>
                  ...
                  {selectionIsProhibited && (
                    <div className="text-sm text-error mt-2">Selezione non valida: non puoi impostare come parent te stesso o un tuo discendente.</div>
                  )}
                </div>
              </div>
            </div>
