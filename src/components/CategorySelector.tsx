...
export function CategorySelector<T extends Category>({
    ...
    function SelectNode({node, depth, isOpen, onToggle}: {
        node: CategoryNode<T>,
        depth: number,
        isOpen: boolean,
        onToggle: () => void
                ...
                <div className="ml-auto">
                    <button className="btn btn-ghost btn-xs p-1" onClick={handleToggle}
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg"
                             className={`h-4 w-4 transform ${isOpen ? 'rotate-90' : ''}`} fill="none"
    ...
    return (
        <>
            <div className={`flex items-center gap-2 ${className ?? ''}`}>
                <button type="button" className="btn btn-outline flex-1 justify-between min-w-0"
                        onClick={() => setOpen(true)} aria-haspopup="dialog" aria-expanded={open}>
                    <div className="flex items-center gap-2 min-w-0 truncate">
                        {selectedNode ? (
                            <>
                                <div className="w-6 h-6 rounded-full border flex items-center justify-center"
                                     style={{backgroundColor: (selectedNode.data as Category).color}}/>
                ...
                {allowRootChoice && (<button type="button" className="btn btn-ghost" onClick={clearSelection}
                        ...
                        <div className="flex items-center justify-between mb-3">
                            ...
                            <div className="flex gap-2">
                                ...
                                <button className="btn btn-primary btn-sm"
                                        ...
                                        disabled={selectionIsProhibited}>Select
                                </button>
                            </div>
                        </div>

                        <div className="mb-3">
                            <input type="text" placeholder="Cerca..." value={filter}
                                ...
                                <div className="card bg-base-100 shadow-sm p-3">
                                        ...
                                        {selectedNode ? (
                                            <>
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <div
                                                            className="font-semibold">{(selectedNode.data as Category).name}</div>
                                                        <div
                                                            className="text-xs opacity-60">{(selectedNode.data as Category).description}</div>
                                                    </div>
                                                </div>
                                    ...
                                    {selectionIsProhibited && (
                                        <div className="text-sm text-error mt-2">Selezione non valida: non puoi
                                            impostare il parent uguale al nodo o a uno dei suoi discendenti.</div>
                                    )}
                                </div>
                            </div>
                        </div>
